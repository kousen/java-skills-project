    ...
    private static void writeEmployeesToJson(List<Employee> employees) throws IOException {
        ...
        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(JSON_FILE))) {
            ...
            for (int i = 0; i < employees.size(); i++) {
                ...
                writer.write("  {");
                writer.newLine();
                writer.write(String.format("    \"id\": %d,", emp.id()));
                writer.newLine();
                writer.write(String.format("    \"name\": \"%s\",", emp.name()));
                writer.newLine();
                writer.write(String.format("    \"salary\": %.2f,", emp.salary()));
                writer.newLine();
                writer.write(String.format("    \"hireDate\": \"%s\"", emp.hireDate().format(DATE_FORMAT)));
                writer.newLine();
                writer.write("  }");
